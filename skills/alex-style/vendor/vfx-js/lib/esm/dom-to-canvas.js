 ...
 * @internal
 */
export default async function getCanvasFromElement(element, originalOpacity, oldCanvas, maxSize) {
    ...
    const canvas = oldCanvas && oldCanvas.width === canvasW && oldCanvas.height === canvasH
        ? oldCanvas
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${fullW}" height="${fullH}">` +
...
async function syncStylesOfTree(el1, el2) {
    ...
    for (const key of Array.from(styles)) {
        ...
        if (/(-inline-|-block-|^inline-|^block-)/.test(key) ||
...
function zeroCollapsingMargins(root) {
    ...
    for (;;) {
        const s = el.style;
        if (Number.parseFloat(s.paddingTop) > 0 ||
    ...
    for (;;) {
        const s = el.style;
        if (Number.parseFloat(s.paddingBottom) > 0 ||
