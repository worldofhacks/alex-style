...
- **Feature-frozen upstream** (last feature release 2023-07; repo alive): with zero deps and a 482-line surface, if a browser change ever bites, the fix is a PATCH IN VENDOR (fancy-fork precedent), not a version float.
...
- **Physics floating/falling/draggable elements** → `fancy` wild tier (`gravity`); that is simulation, not hover depth.
...
- Upstream is feature-frozen: expect no fixes. A future browser regression gets a `PATCHED(alex-style)` in-vendor fix with fail-loud anchors (fancy precedent), never a float to an unaudited version.
