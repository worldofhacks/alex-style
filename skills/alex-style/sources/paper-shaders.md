...
6. **WebGL2 required — CSS fallback background is mandatory** on the container (the constructor THROWS without WebGL2; the react component renders a bare canvas). The fallback color should be the shader's own colorBack so failure is invisible.
7. **`minPixelRatio` defaults to 2** (renders 2x even on 1x screens for antialiasing) — lower it (`minPixelRatio={1}`) on mobile for full-viewport shaders; budget: full-screen 4K-class pixel counts are the default ceiling (`maxPixelCount`).
