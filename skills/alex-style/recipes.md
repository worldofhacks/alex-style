...
3. Hero background: subtle beams/grid, NOT WebGL — `grep -iE "beam|grid|dot" vendor/_index/components.tsv` (magicui `animated-grid-pattern`, `warp-background`; kokonutui alternatives).
...
3. Tile hover: `TiltedCard-TS-TW` OR `PixelTransition-TS-TW` — one hover vocabulary across all tiles, never mixed.
...
3. Animated hero bg (optional): a light Paper Shaders preset from `vendor/_index/paper-shaders.tsv` at low speed (`sources/paper-shaders.md`), or shadergradient `cottonCandy` / `mint` (`_index/shadergradient-presets.tsv`); vanta's light-native rows are `clouds` (bg 0xffffff) and `ripple` (0xf6f6f6). One canvas max, per #ambient-bg.
